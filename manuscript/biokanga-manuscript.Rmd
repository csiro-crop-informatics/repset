...
Even though versions of the required software were specifically stated, re-running of the analysis would require downloading and installing all the required software tools.
...
- Docker automated build configured for each benchmarked tool
- Additional docker container images generated for auxiliary tool-sets required for benchmarking, including, but not limited to:
  + RNA-seq benchmarking toolkit [@Baruzzo2016] modified in https://github.com/rsuchecki/biokanga_benchmark
  + DNA-seq benchmarking toolkit (automated)

The procedure for adding or updating container images involves creating a dedicated tool/version branch in the main repository.
Changes committed to that branch are captured in a container image in the associated Docker Hub repository.
This is done through by an automated build triggered by the push to that branch.
...
An auxiliary tool may have to be added to the container to facilitate its use within a workflow e.g.
to aid collection of execution statistics. In such case the version tag remains unchanged
yet the underlying container image has been replaced.
...
This perma-tags are then used when referencing a container in workflow configuration files.

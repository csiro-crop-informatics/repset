...
The procedure for adding or updating container images involves creating a dedicated tool/version (e.g. `samtools/1.9`) branch in the main repository.
...
using semantic versioning e.g. `v1.0.5` the same style may be (and often is (REF) be used for scientific workflows.
...
workflow run metadata and, subject to space limitations, the generated results.
...
Given (optional) access to an authentication token, our workflow, on

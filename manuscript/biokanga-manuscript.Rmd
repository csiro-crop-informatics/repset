...
while the rest of the workflow remains unchanged, with the exception of some downstream processes
which aggregate data produced by all of the evaluated tools.
The container per tool or perhaps per branch of the workflow may also be advantageous
for computationally intensive or long-running workflows as this approach limits the need to
re-run parts of the pipeline due to global software environment being modified.

We provide a Docker Hub repository for each of the benchmarked tools.
Each repository is configured to automatically build a version of the container
for a tool given an appropriate trigger in our workflow repository.
...
and the DNA-seq benchmarking toolkit [@Brinda2015] (automated).

The procedure for adding or updating individual container images
involves creating a dedicated tool/version (e.g. `samtools/1.9`) branch in the main repository.
...
These perma-tags are then used when referencing a container in workflow configuration files.
...
Although substantial, all these overheads can be seen as investments. Carefully designed portable workflow configuration
can be easily re-used such that subsequent workflows can be developed much quicker.

...
RUN apt-get update && apt-get install -y wget build-essential automake make unzip pigz ruby-full python2.7
...
RUN wget https://github.com/rsuchecki/biokanga_benchmark/archive/v${EVALU_VERSION}.tar.gz \
  ...
  && gem install erubis \
  && mkdir -p /project/itmatlab/aligner_benchmark/dataset/human/{genome,dataset_t{1,2,3}r{1,2,3}} /project/itmatlab/aligner_benchmark/tool_results /project/itmatlab/aligner_benchmark/statistics/
